...
class RSDProblem(BaseProblem):
    ...
    def __init__(self, n: int, k: int, w: int, q: int, z: int, **kwargs):  # Fill with parameters
        ...
        if (q-1)%z != 0:
            raise ValueError("z must be ?")
        self.parameters[RSD_CODE_LENGTH] = n
        self.parameters[RSD_CODE_DIMENSION] = k
        self.parameters[RSD_ERROR_WEIGHT] = w
        self.parameters[RSD_SYNDROME_FIELD_SIZE] = q
        self.parameters[RSD_ERROR_FIELD_SIZE] = z
    ...
    def __repr__(self):
        ...
        rep = "syndrome decoding problem with (n,k,w) = " \
              + "(" + str(n) + "," + str(k) + "," + str(w) + ") over Finite Field of size " + str(q) + "and Restricted Set of size " +str(z)
